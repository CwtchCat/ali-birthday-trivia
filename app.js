...
function resultMessage(score, name) {
    const pct = score / MAX_SCORE;
    if (pct >= 0.9)
        return `Incredible, ${name}! You really know them inside out. Gold star performance! ⭐`;
    if (pct >= 0.7)
        return `Well done, ${name}! You clearly know them pretty well — a solid effort!`;
    if (pct >= 0.5)
        return `Not bad, ${name}! Half the answers were spot on — there's still plenty to learn about them!`;
...
function attachListeners() {
    ...
    switch (state.page) {
        case "start":
            (_a = document.getElementById("btn-proceed")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", () => {
                state.page = "name";
                render();
            });
            break;
        case "name": {
            ...
            nameInput.addEventListener("keydown", (e) => {
                if (e.key === "Enter" && state.playerName.trim() !== "")
                    beginQuiz();
            });
        ...
        case "quiz": {
            ...
            document.querySelectorAll(".btn-judgment").forEach((btn) => {
                btn.addEventListener("click", () => {
                    const judgment = btn.dataset.judgment;
                    applyJudgment(judgment);
                });
            });
            (_b = document.getElementById("btn-back")) === null || _b === void 0 ? void 0 : _b.addEventListener("click", () => {
                if (state.currentQuestion > 0) {
                    state.currentQuestion--;
                    render();
                }
            });
            (_c = document.getElementById("btn-next")) === null || _c === void 0 ? void 0 : _c.addEventListener("click", () => {
        ...
        case "results":
            (_d = document.getElementById("btn-restart")) === null || _d === void 0 ? void 0 : _d.addEventListener("click", () => {
                resetState();
                render();
            });
            break;
